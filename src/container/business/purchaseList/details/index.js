...
class buyDetail extends Component {
    ...
    fetchDetails = () => {
        ...
        fetchPost(api.buyDetails, params).then(res => {
            const code = res.code;
            if (code === '1000') {
    ...
    fetchOutputDynamic = () => {
        ...
        fetchDynamic(orderCode, 4, code).then(res => {
            let list = res.data;
    ...
    render() {
        ...
        switch (details.reason) {
            case '0':
                ...
                break;
            case '1':
                details.reason = '商品送错/多送/近效期等退回';
                break;
            case '2':
                details.reason = '有质量问题退回';
                break;
            case '3':
                details.reason = '配送过程损坏退回';
                break;
            case '4':
                details.reason = '商品过期退回';
                break;
            case '5':
                details.reason = '其它';
                break;
        ...
        return (
            <div className="m-output-detail-wrap">
                ...
                <div className="m-output-detail-inner">
                    <Title title="采购退货订单详情"
                           subTitle={{assgin1: '采购退货计划单', assgin2: '采购退货单详情', Link: '/m/business/purchaseList'}}/>
